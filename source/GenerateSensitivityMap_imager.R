...
sens.xy      <- load.image(path.to.png)
# avoid asymmetric transverse map
if(diff(dim(sens.xy[,,1,1]))==0) n.px.png <- length(sens.xy[,1,1,1]) else stop("Asymmetric XY-map!")
...
unrolled.sens.xy <- c(resize(sens.xy, size.xy, size.xy)[,,1,1])
